    ...
    useEffect(() => {
        ...
        const getUserSnap = onSnapshot(userRef, async(userSnap) => {
            if(userSnap.exists()){
                ...
                const followerData = await getUsersData(userSnap.data().followers || [])
                setFollowers(followerData as FollowInfo[]);

                const followingData = await getUsersData(userSnap.data().following || []);
                            ...
                            {tabIndex === 0 && (
                                <ul className='SearchArea'>
                                    {followers.length > 0 ? followers.map(follower => (
                                        ...
                                        >
                                            <Link to={follower.id === user?.uid ? "/Profile" : `/user/${follower.id}`}>
                                ...
                                <ul className='SearchArea'>
                                    {following.length > 0 ? following.map(followingUser => (
                                        <li
                                            ...
                                            style={{ display: 'flex', gap: '10px', alignItems: 'center', justifyContent: 'space-between', padding: '0.5em' }}
                                        >
                                            <Link to={followingUser.id === user?.uid ? "/Profile" : `/user/${followingUser.id}`}>
