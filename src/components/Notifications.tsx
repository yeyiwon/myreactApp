        ...
        if (user) {
            ...
            const filterNotification = onSnapshot(notificationQuery, async (snapshot) => {
                ...
                for (const filterNotisnap of snapshot.docs) {
                    ...
                    /* 타입 변환을 반복문 돌리면서 하는거는 snapshot.docs 얘는 현재 배열로가지고있고, ex) 유저가 받은 알림이 4개일 때 4개인 것만 뜸
                    타입 변환
                ...
                // 읽지 않은 알림 처리
                const unreadDocs = snapshot.docs.filter(unread => unread.data().isRead === false);
                // 읽지 않은 거만 필터링 해서 
                if (unreadDocs.length > 0) {
                    // unreadDocs 안 읽은 거만 뽑아낸 거의 길이가 0 보다 클 때 // useEffect의 특성을 살려  [user]) 이거가 이 페이지에 들어왔다면 ! 즉 ! [user] 가 있을 때 읽음으로 처리 
                    const batch = writeBatch(db);
                    unreadDocs.forEach(unread => { // 읽지 않은 메세지 사악 돌고
                        batch.update(unread.ref, { isRead: true });
                    });
                    await batch.commit();
                        /* -- 중요한 오류 노트 ! --  
                        if (unreadDocs.length > 0)  이렇게 안 읽은 메세지 isfalse의 갯수를 length로 받았기 때문에 length로 batch update를 하려고 했으나 
                        오류가 야무지게 나서 보니까 
                        isRead가 false 인 갯수를 구하는 건 length고 wirteBatch가 false => true로 바꾸어주는 작업은 문서작업이기 때문에 ref로 처리한다 

                        문서 갯수 확인(length) 후 일괄 업뎃(ref)
